...
def export_binary(model, savpath = "./model_binaries"):
    ...
    for layer in model.layers[:-1]: # the final layer is a dense top
        ...
        # Concatonate U to W
        wi = np.hstack((U[:,:units].T, W[:,:units].T)).flatten()
        wf = np.hstack((U[:,units:units*2].T, W[:,units:units*2].T)).flatten()
        wc = np.hstack((U[:,units*2:units*3].T, W[:,units*2:units*3].T)).flatten()
        wo = np.hstack((U[:,units*3:].T, W[:,units*3:].T)).flatten()
